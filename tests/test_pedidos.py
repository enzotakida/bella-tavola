...
def test_criar_pedido_valido_retorna_campos_esperados():
...
def test_criar_pedido_calcula_valor_total_corretamente():
...
def test_criar_pedido_com_quantidade_negativa_retorna_422():
...
def test_criar_pedido_com_prato_indisponivel_retorna_400():
    # prato id 3 (Lasanha Bolonhesa) está marcado como disponivel: False no database
    pedido = {"prato_id": 3, "quantidade": 1}
    response = client.post("/pedidos/", json=pedido)
    assert response.status_code == 400

def test_pedido_criado_preserva_observacao():
